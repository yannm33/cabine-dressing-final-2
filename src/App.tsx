...
import { TranslationKey, PoseKey } from './lib/translations';
...
import { HairstyleKey, hairstyleOptions } from './hairstyles';
...
const App: React.FC = () => {
  ...
  const handleApplyHairstyle = useCallback(async (hairstyleKey: HairstyleKey) => {
      ...
      setActiveGenerator(null);
      setActiveTab('outfit'); // Switch to outfit tab to see the new layer
  }, [performImageModification, t]);
        ...
        {!hasModel ? (
          ...
          >
            <div className="w-full">
               <Header isMockMode={USE_MOCK_API} isStartScreen={true} />
            </div>
          ...
          <motion.div
            ...
            {...viewAnimation}
          >
            <Header isMockMode={USE_MOCK_API} isStartScreen={false} />
            <main className="flex-grow flex flex-col gap-4 p-4 overflow-y-auto">
                {/* --- NEW PRIMARY NAVIGATION --- */}
                <div className="flex-shrink-0 bg-white rounded-xl shadow-sm border border-gray-200/80 p-2">
                    <div className="flex justify-center items-center gap-2 sm:gap-4 flex-wrap">
                        <button 
                            onClick={() => setActiveTab('outfit')} 
                            className="bg-orange-500 text-black font-semibold px-4 sm:px-6 py-2 rounded-lg text-sm transition hover:bg-orange-600 active:scale-95">
                            {t('myOutfit')}
                        </button>

                        <div className="relative" ref={hairstyleDropdownRef}>
                            <button 
                                onClick={() => setIsHairstyleDropdownOpen(prev => !prev)}
                                className="bg-orange-500 text-black font-semibold px-4 sm:px-6 py-2 rounded-lg text-sm transition hover:bg-orange-600 active:scale-95 flex items-center gap-2">
                                {t('hairstyle')}
                            ...
                            {isHairstyleDropdownOpen && (
                                <motion.div 
                                    ...
                                    animate={{ opacity: 1, y: 0 }}
                                    exit={{ opacity: 0, y: -10 }}
                                    className="absolute top-full mt-2 w-56 bg-white rounded-lg shadow-lg border z-20 max-h-60 overflow-y-auto"
                                >
                                    <div className="p-1">
